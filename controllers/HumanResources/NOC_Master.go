// Package controllers for the Noc_master handles HTTP request routing, authentication,
...
// APIResponseforNOCmaster defines the standard structure of the API response.
type APIResponseforNOCmaster struct {
	Status  int         `json:"Status"`
	Message string      `json:"message"`
	Data    interface{} `json:"Data"`
}

// GetNOCmaster handles the HTTP GET request to fetch NOCmaster data for a Employees.
//
// Flow:
//  1. Authenticates the request using token/IP validation
//  2. Logs the incoming request
//  3. Fetches NOCmaster data from the database based on Employeeid
...
func GetNOCmaster(w http.ResponseWriter, r *http.Request) {
	...
	loggedHandler := auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Step 4: Retrieve NOCmaster data from DB
		NOCmasterData, totalCount, err := databasehumanresources.NOCmasterdatabase(w, r)
		if err != nil {
		...
		var response APIResponseforNOCmaster
		{
			// Case: Successful data retrieval
			response = APIResponseforNOCmaster{
				Status:  200,
				Message: "Success",
				Data: map[string]interface{}{
					"No Of Records": totalCount,
					"Records":       NOCmasterData,
				},
			}
		}
