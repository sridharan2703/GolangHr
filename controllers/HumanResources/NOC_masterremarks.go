// Package controllers for the Noc_masterremarks handles HTTP request routing, authentication,
...
// APIResponseforNOCmasterremarks defines the standard structure of the API response.
type APIResponseforNOCmasterremarks struct {
	Status  int         `json:"Status"`
	Message string      `json:"message"`
	Data    interface{} `json:"Data"`
}

// GetNOCmasterremarks handles the HTTP GET request to fetch NOCmaster data for a Employees.
//
// Flow:
//  1. Authenticates the request using token/IP validation
//  2. Logs the incoming request
//  3. Fetches NOCmasterremarks data from the database based on Employeeid
...
func GetNOCmasterremarks(w http.ResponseWriter, r *http.Request) {
	...
	loggedHandler := auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Step 4: Retrieve NOCmasterremarks data from DB
		NOCmasterremarksData, totalCount, err := databasehumanresources.NOCmasterremarksdatabase(w, r)
		if err != nil {
		...
		var response APIResponseforNOCmasterremarks
		{
			// Case: Successful data retrieval
			response = APIResponseforNOCmasterremarks{
				Status:  200,
				Message: "Success",
				Data: map[string]interface{}{
					"No Of Records": totalCount,
					"Records":       NOCmasterremarksData,
				},
			}
		}
