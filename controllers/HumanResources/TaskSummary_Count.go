// Package controllers for the Tasksummarycount handles HTTP request routing, authentication,
...
// APIResponsefortasksummarycount defines the standard structure of the API response.
type APIResponsefortasksummarycount struct {
	Status  int         `json:"Status"`
	Message string      `json:"message"`
	Data    interface{} `json:"Data"`
}

// GetTasksummarycount handles the HTTP GET request to fetch Tasksummarycount data for a Employees.
//
// Flow:
//  1. Authenticates the request using token/IP validation
//  2. Logs the incoming request
//  3. Fetches Tasksummarycount data from the database based on Employeeid
...
func Gettasksummarycount(w http.ResponseWriter, r *http.Request) {
	...
	loggedHandler := auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Step 4: Retrieve Tasksummarycount data from DB
		TasksummarycountData, totalCount, err := databasehumanresources.Tasksummarydatabase(w, r)
		if err != nil {
		...
		var response APIResponsefortasksummarycount
		{
			// Case: Successful data retrieval
			response = APIResponsefortasksummarycount{
				Status:  200,
				Message: "Success",
				Data: map[string]interface{}{
					"No Of Records": totalCount,
					"Records":       TasksummarycountData,
				},
			}
		}
