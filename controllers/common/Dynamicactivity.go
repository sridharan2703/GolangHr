...
//new1111111111111111111111111111222222222222222222222222222
...
type APIResponseforDynamicActivitySequence struct {
...
func DynamicActivitySequence(w http.ResponseWriter, r *http.Request) {
	...
	loggedHandler := auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Step 4: Hardcoded data
		DefaultRoleNameData := []map[string]interface{}{
			{
				"activitySeqNo":  1,
				"assignedToRole": "Initiator",
				"isTaskApproved": 1,
			},
			{
				"activitySeqNo":  2,
			...
			{
				"activitySeqNo":  3,
				"assignedToRole": "Admin2 Dealing Asisstant",
				"isTaskApproved": 0,
			},
		}
		totalCount := len(DefaultRoleNameData)

		// Step 5: Structure API response
		response := APIResponseforDynamicActivitySequence{
			Status:  200,
			Message: "Success",
