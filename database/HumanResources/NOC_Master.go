...
func NOCmasterdatabase(w http.ResponseWriter, r *http.Request) ([]modelshumanresources.NOCmasterstructure, int, error) {
	...
	rows, err := db.Query(modelshumanresources.MyQueryNOCmaster, Taskid)
	if err != nil {
	...
	NOCmasterapi, err := modelshumanresources.RetrieveNOCmaster(rows, Taskid)
	if err != nil {
		return nil, 0, fmt.Errorf("error retrieving data: %v", err)
	}

	// Count total records returned
	totalCount := len(NOCmasterapi)
	return NOCmasterapi, totalCount, nil
}
